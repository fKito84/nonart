...
<div class="space-y-16 pb-12">
        ...
        <div class="bg-[#282119] border border-[#3d352b] rounded-2xl p-10 shadow-xl">
            ...
            <ul class="flex flex-col space-y-8 items-center text-center">
                ...
                <li>
                    ...
                    <span class="text-xl text-[#f2ede6] tracking-[1px] font-light">{{ $usuario->phone ?? 'No has deixat informació' }}</span>
                </li>
            </ul>  
            ...
            <div class="mt-12 pt-8 border-t border-[#3d352b] flex justify-center">
                <form method="POST" action="{{ route('logout') }}" class="w-full max-w-sm">
                    @csrf
                    <button type="submit" class="block w-full text-center py-4 border border-[#fb2c36] text-[#fb2c36] rounded-xl text-sm uppercase tracking-widest hover:bg-[#fb2c36] hover:text-white transition-colors">
                        Tancar Sessió
                    </button>
                </form>                       
            </div>
    ...
    <div class="max-w-5xl mx-auto mt-20 px-4">
        <h2 class="text-4xl md:text-5xl mb-12 text-center border-b border-[#3d352b] pb-6 uppercase tracking-widest">Les meves adquisicions</h2>
        
        <div class="space-y-10">
            
            @foreach($usuario->reserva_tallers as $reserva)
            <div class="bg-[#282119] border border-[#3d352b] rounded-2xl p-8 flex flex-col md:flex-row items-center md:items-start gap-10 hover:border-[#c9973a] transition-all duration-300">
                ...
                <div class="flex-grow pt-2">
                    <div class="flex justify-between items-start mb-4">
                        <span class="px-4 py-1 bg-[#332b22] text-[#c9973a] text-[12px] uppercase tracking-widest rounded border border-[#c9973a]/30">Taller</span>
                        <div class="text-[#a0937f] text-sm">{{ $reserva->created_at->format('d/m/Y') }}</div>
                    </div>
                    <h3 class="text-4xl text-[#f2ede6] mb-4">{{ $reserva->taller->titol }}</h3>
                    <p class="text-[#00c950] text-lg flex items-center gap-3">
                        ...
                        Reserva confirmada ({{ $reserva->num_persones }} places)
                    </p>
                </div>
            </div>
            @endforeach
            ...
            <div class="bg-[#282119] border border-[#3d352b] rounded-2xl p-8 flex flex-col md:flex-row items-center md:items-start gap-10 hover:border-[#c9973a] transition-all duration-300">
                <div class="w-full md:w-56 h-56 flex-shrink-0 rounded-xl overflow-hidden bg-[#1e1914]">
                    <img src="{{ asset('images/obras/' . $venda->obra->imatge) }}" class="w-full h-full object-cover">
                </div>
                
                <div class="flex-grow pt-2">
                    <div class="flex justify-between items-start mb-4">
                        <span class="px-4 py-1 bg-[#332b22] text-[#f2ede6] text-[12px] uppercase tracking-widest rounded border border-[#3d352b]">Obra</span>
                        <div class="text-[#a0937f] text-sm">{{ $venda->data_venda->format('d/m/Y') }}</div>
                    </div>
                    <h3 class="text-4xl text-[#f2ede6] mb-4">{{ $venda->obra->titol }}</h3>
                    <p class="text-[#a0937f] text-lg flex items-center gap-3">
                        <span class="w-3 h-3 rounded-full bg-[#a0937f]"></span> 
                        Pagament completat ({{ $venda->preu_final }}€)
                    </p>
                </div>
            </div>
            @endforeach
        ...
        <div class="mt-16 text-center">
            <a href="/obras" class="inline-block py-5 px-12 bg-[#c9973a] text-[#1e1914] rounded-xl text-[16px] font-bold uppercase tracking-[3px] hover:bg-[#e6ae42] transition-all shadow-xl">
                Nova Compra
            </a>
        </div>
    </div>
